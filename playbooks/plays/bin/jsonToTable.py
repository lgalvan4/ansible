...
for key in da['updates']:
    ...
    da['updates'][str(key)]['categories'] = da['updates'][str(
        key)]['categories'][0]
...
for fvalue in duckingUpdates:
    ...
    if fvalue['categories'] in catgories_dict:
        catgories_dict[fvalue['categories']] = catgories_dict.get(
            fvalue['categories'])+1
    else:
...
patches, texts = ax1.pie(contadores, explode=tuple(
    explod), colors=tuple(fcolors), startangle=90)
